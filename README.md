...
カスタム版はボードマネージャ上で **「(Trimmed)」** という接尾辞付きで表示されます。
パッケージ名を公式版と別名（例: `rp2040-trimmed`）にしているため、公式のボードマネージャーURLと本リポジトリのURLを両方登録しても衝突せず、安全に共存・比較できます。
...
`platforms.yml` に登録されているプラットフォームのすべてのボード名と内部ID（Board ID）は、**[AVAILABLE_BOARDS.md](./AVAILABLE_BOARDS.md)** に一覧化されます。
この一覧から必要なボードの内部IDを見つけ、リポジトリ直下の `platforms.yml` の `targets` を書き換えてプッシュするだけで、指定したボードだけに絞られたパッケージが構築されます。

### 新しいプラットフォームを追加する場合

もし全く別のサードパーティコアを追加したい場合は、リポジトリ直下の `platforms.yml` の `platforms` に新しい `platform` 名と公式の `json_url` を追加します。
...
このシステムは、フォークした際や新しいコアを追加した際に、`platforms.yml` 内の `platform` の名前に基づいて配信用のJSONファイルを自動生成します。
...
例えば、`platforms.yml` の設定が以下のようになっている場合：
```yaml
          - platform: rp2040
            json_url: ...
```
...
設定が完了すると、これ以降は `platforms.yml` や `scripts/` 配下に変更を加えてプッシュするたびに、自動的にビルドとリリース作成のワークフローが走るようになります。
